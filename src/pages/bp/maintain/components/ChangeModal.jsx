...
import PersonCertificationAddModal from '@/pages/bp/maintain_details/components/PersonCertificationAddModal';
...
class ChangeModal extends Component {
        ...
        api.bp.getBPOrgCertification(recordMsg.id).then(res => {
          this.setState({
            ...
            specialInvoice: res.organizationCertification
              ? parseInt(res.organizationCertification.specialInvoice) === 1
              : false,
          });
        });
      }
  ...
  groupUsaShow = () => (
    <FormItem label="名称">
      ...
      <a href="#" onClick={event => this.updetaUsaGroup(event)}>
        修改
      </a>
    </FormItem>
  );
  ...
  groupUsaInput = () => {
    const { form } = this.state;
    const { getFieldDecorator } = form;
    return <FormItem label="名称">{getFieldDecorator('name')(<AddressGroup />)}</FormItem>;
  };
        ...
        <List.Item key={item.id}>
          ...
          >
            <div style={{ marginBottom: '.8em' }}>
              <Badge
                status={renzhengMap[item.invoicePartyCode].value}
      ...
      modelContent = (
        <>
          <Form {...formItemLayout}>
            ...
            <Form.Item label="认证图片">{getFieldDecorator('idenImg')(uploadModal)}</Form.Item>
            <Divider style={{ margin: 0 }} />
            <Button
              htmlType="submit"
              type="primary"
