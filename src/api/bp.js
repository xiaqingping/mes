...
export default {
  ...
  customerSalesOrderBlock(id) {
    return request(`/businessPartners/v1/${id}/customer/salesOrderBlock`, { method: 'POST' });
  },
  // 根据业务伙伴ID激活客户销售接口
  ...
  checkBPFields(data) {
    return request('/businessPartners/v1/check', { method: 'POST', data });
  },
  ...
  getPersonBPCarryInvoicePartys(params) {
    return request('/businessPartners/v1/person/invoicePartys', { params });
  },
  // 查询所有客户类型为人员的业务伙伴
  getPersonBP(params) {
  ...
  changeContactInfoAnswerVerify(id, data) {
    return request(`/businessPartner/v1/${id}/verifyRecords/changeContactInfo/answerSubmission`, { method: 'POST', data });
  },
  // 人工辅助变更已验证手机邮箱流程中发送新手机验证短信接口（根据验证编号+手机产生验证码并发送）
  changeContactInfoNewMobileVerifyCodeSendingVerify(verifyRecordId, data) {
    return request(`/businessPartner/v1/verifyRecords/${verifyRecordId}/changeContactInfo/newMobileVerifyCodeSending`, { method: 'POST', data });
  },
  // 人工辅助变更已验证手机邮箱流程中发送新邮箱验证邮件接口（根据验证编号+邮箱产生验证码并发送）
  changeContactInfoNewEmailVerifyCodeSendingVerify(verifyRecordId, data) {
    return request(`/businessPartner/v1/verifyRecords/${verifyRecordId}/changeContactInfo/newEmailVerifyCodeSending`, { method: 'POST', data });
  },
  // 人工辅助变更已验证手机邮箱流程中新手机邮箱验证码验证接口（根据验证编号验证验证码是否正确）
  changeContactInfoNewMobileVerifyCodeVerificationVerify(verifyRecordId, data) {
    return request(`/businessPartner/v1/verifyRecords/${verifyRecordId}/changeContactInfo/newMobileVerifyCodeVerification`, { method: 'POST', data });
  },
  ...
  changeContactInfoNewEmailVerifyCodeVerificationVerify(verifyRecordId, data) {
    return request(`/businessPartner/v1/verifyRecords/${verifyRecordId}/changeContactInfo/newEmailVerifyCodeVerification`, { method: 'POST', data });
  },
};
