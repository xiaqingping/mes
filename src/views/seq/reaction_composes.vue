...
<template>
  <div class="page-content">
    ...
    <vxe-grid
      ...
      auto-resize
      height="600"
      ref="reactionComposes"
      :loading="reactionComposes.loading"
      :columns="reactionComposes.columns"
      :pager-config="reactionComposes.pagerConfig"
      :data.sync="reactionComposes.tableData"
      @page-change="({pageSize, currentPage}) => this.$utils.tablePageChange({pageSize, currentPage, table: reactionComposes, callback: handleSearch})">
    </vxe-grid>
  </div>
</template>
...
export default {
  ...
  methods: {
    // 设置表格列属性
    setColumn () {
      const tableName = 'reactionComposes';
      ...
      const columns = [
        { type: 'radio', width: 40 },
        { type: 'index', width: 40 },
        { title: '反应板号', dataIndex: 'code' },
        { title: '测序类型', dataIndex: 'reactions' },
        { title: '测序仪', dataIndex: 'seqdeviceId' },
        { title: '状态', dataIndex: 'status' },
        { title: '测序点', dataIndex: 'seqfactoryId' },
        { title: '上机人姓名', dataIndex: 'ondeviceName' },
        { title: '上机时间', dataIndex: 'ondeviceDate' },
        { title: '实际上机开始时间', dataIndex: 'ondeviceStartDate' },
        { title: '实际上机结束时间', dataIndex: 'ondeviceEndDate' },
    ...
    handleSearch () {
      ...
      this.$api.reaction.getReactioncomposes(params, true).then((data) => {
