...
<template>
  <div class="page-content">
    ...
    <vxe-grid
      highlight-hover-row
      auto-resize
      :ref="resistanceTable.ref"
      :loading="resistanceTable.loading"
      :columns="resistanceTable.columns"
      :pager-config="resistanceTable.pagerConfig"
      :data.sync="resistanceTable.tableData"
      :edit-rules="resistanceTable.editRules"
...
export default {
  ...
  methods: {
    // 设置表格列属性
    setColumn () {
      const tableName = 'resistanceTable';
    ...
    handleSearch () {
      ...
      this.$api.sampletype.getSampleResistance(params, true).then((data) => {
    ...
    // 删除
    handleCancel ({ row }) {
      this.$api.sampletype.cancelSampleResistance(row.id).then(() => {
        this.handleSearch();
      });
    },
    // 保存
    handleSave ({ row, rowIndex, tableName, xTable }) {
      xTable.validate(row).then(() => {
        ...
        // 新增
          this.$api.sampletype.addSampleResistance(row).then(() => {
            this.handleSearch();
          });
        }
      }).catch(() => {
