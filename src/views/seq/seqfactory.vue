  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return this.$api.seqfactory.getSeqfactory(params, true).then(res => {
          return {
            data: res.rows,
            page: params.page,
            total: res.total
  ...
  methods: {
    createColumnDefs () {
      ...
      this.columns = [
        ...
        { title: 'SAP工厂', dataIndex: 'factory', customRender: function (text, record, index) { return formatter(basic.factorys, text, 'code', 'text'); } },
        { title: '仓库', dataIndex: 'storageCode', customRender: function (text, record, index) { return formatter(basic.storages, text, 'code', 'text'); } },
