          ...
          <a-col :xxl="4" :xl="4" :md="4" :sm="24">
            <a-form-item label="年：">
              <a-select v-decorator="['year', {initialValue : ''}]">
                ...
                <a-select-option v-for="year in $store.state.pay.year" :value="year.id" :key="year.id">{{ year.name }}</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :xxl="4" :xl="4" :md="4" :sm="24">
            <a-form-item label="月：">
              <a-select v-decorator="['month', {initialValue : ''}]">
                <a-select-option value="">全部</a-select-option>
                <a-select-option v-for="month in $store.state.pay.month" :value="month.id" :key="month.id">{{ month.name }}</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :xxl="4" :xl="4" :md="4" :sm="24">
...
export default {
  ...
  data () {
    return {
      ...
      addcolumns: [
        { title: '编号', dataIndex: 'itemList[0]', width: '25%' },
        { title: '名称', dataIndex: 'itemList[0]', width: '25%' },
        {
          title: '类型',
          dataIndex: 'itemList',
          customRender: function (text) {
            if (text === 1) return '工资项目';
            else if (text === 2) return '扣款项目';
            else if (text === 3) return '代发项目';
            else if (text === 4) return '代缴项目';
          },
          width: '25%'
        },
  ...
  methods: {
    ...
    setColumn () {
      ...
      const columns = [
        ...
        { label: '员工名称', prop: 'employeeName', editRender: { name: 'input' } },
    ...
    handleSearch () {
      ...
      this.$api.pay.getPay(params, true).then((data) => {
